...
# variables setting
{
  ...
  for (i in 1:(length(lag_days)-1)) {
    predictor_list <- rbind(predictor_list, combn(lag_days, m = i, function(x) as.vector(lag_days %in% x)))
  }
...
write.csv(model_result, file = paste0(save_path, "/ts_model_result.csv"))
